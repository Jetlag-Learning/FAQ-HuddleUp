...
export const ChatWidget: React.FC<ChatWidgetProps> = ({ embedded = false, defaultOpen = false }) => {
  ...
  const detectSchedulingIntent = (text: string): boolean => {
    const exactSchedulingPhrases = [
      ...
      'schedule a time', 'book a time', 'set up a time', 'arrange a time',
      'schedule a time to discuss', 'book a time to discuss', 'set up a time to discuss',
      'help you schedule', 'happy to help you schedule', 'like to schedule',
      'would you like me to help you schedule', 'i\'d be happy to help you schedule'
    ];
    
    const lowerText = text.toLowerCase();
    
    // Enhanced detection for scheduling intent
    return exactSchedulingPhrases.some(phrase => lowerText.includes(phrase)) ||
           // Allow flexibility for common scheduling patterns
           (lowerText.includes('schedule') && (lowerText.includes('meeting') || lowerText.includes('call') || lowerText.includes('time') || lowerText.includes('discuss'))) ||
           (lowerText.includes('book') && (lowerText.includes('meeting') || lowerText.includes('call') || lowerText.includes('time'))) ||
           (lowerText.includes('set up') && (lowerText.includes('meeting') || lowerText.includes('call') || lowerText.includes('time'))) ||
           ...
           // Catch pricing-related scheduling prompts
           (lowerText.includes('discuss') && lowerText.includes('pricing')) ||
           (lowerText.includes('help you schedule') && lowerText.includes('derek'));
